    ...
    <section id="about" className="py-20 px-4 md:px-8" ref={sectionRef}>
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <motion.div className="w-16 h-1 bg-accent mx-auto rounded-full" variants={fadeInUp} />
        </motion.div>
            ...
            <div className="relative">
              ...
              <div className="space-y-8">
                {about.education.map((edu, i) => (
                  <motion.div key={i} className="relative pl-10" variants={fadeInUp}>
                    <TimelineDot />
                    {edu.period && (
                      <time className="text-accent text-xs font-medium" dateTime={edu.datetime}>
                        {edu.period}
                      </time>
                    )}
                    <h4 className="text-text font-semibold mt-1 text-sm md:text-base">
                      {edu.title}
                    </h4>
                    <p className="text-muted text-sm">{edu.institution}</p>
                    {edu.detail && (
                      <p className="text-muted text-xs mt-1 opacity-75">{edu.detail}</p>
                    )}
                  </motion.div>
                ))}
              </div>
                ...
                {experience.map((exp, i) => (
                  <motion.div key={i} className="relative pl-10" variants={fadeInUp}>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <time className="text-accent text-xs font-medium" dateTime={exp.datetime}>
                        {exp.period}
                      </time>
                      {exp.isCurrent && exp.startDate && (
                        <span className="text-muted text-xs opacity-75">
